...
- **홈** — 발견한 세계 · 발견한 원리 두 칸이 나란히, 그 아래 보석함.
  한 화면에 들어온다. 실기기 브라우저의 무대 높이(660px 안팎)에 맞춰
  홈에서만 조판을 좁힌다 (`.ex-page--home`)
...
손가락이 닿는 자리는 어디든 44px 이상이다.
보이는 크기는 조판을 따르고, 모자란 만큼만 가짜 요소로 넓힌다
(`.ex-back::after` · `.ex-jem::after` · `.quiet-action::before`).
그래서 화면은 그대로인데 눌리는 범위만 넓다.

좁은 화면(380px 아래)에서는 목록 한 줄의 그림 칸을 줄여 글에 자리를 준다.
원리 목록이 2409px 에서 2052px 로 줄어든다.
...
상시로 움직이는 것은 **강과 구름 둘뿐**이다.
자리를 옮기는 것이 아니라, 죽어 있지 않다는 정도다.

| 요소 | 하는 일 | 주기 |
|---|---|---|
| 구름 | 제자리에서 오간다 (늘) | 104초 → 고리 뒤 76초, 폭 ±12px |
| 구름 | 아주 옅게 부풀었다 가라앉는다 | 38초 |
| 구름 | 안에서 숨이 오간다 | 26초 |
| 강 | 물빛이 천천히 들고 난다 | 27초 |
| 강 | 빛 한 줄이 먼 곳에서 앞으로 내려온다 | 23초 |
| 강 | 표면이 잘게 반짝인다 | 9초 |

**비는 들어 있지 않다.** 비는 한 질문에서 잠깐 내렸다 새싹이 올라오며
그치는 것이지, 세계가 늘 하는 일이 아니다.

강의 흐름과 반짝임은 강 그림 자체를 마스크로 써서 물길 안에서만 지나간다.
새 그림을 쓰지 않는다.

구름의 떠다님은 CSS 가 맡는다. 타이머를 두지 않고, 한쪽으로 밀려나지도 않는다.

무엇이 살아나는지는 `world.js` 의 `LIVING` 한 군데에 있다.
고리를 더할 때는 여기에 이름 하나를 더하고
`cycles.js` 의 `animates` 에 그 이름을 적으면 된다.

움직임을 줄여 달라고 한 기기에서는 전부 멈춘다
