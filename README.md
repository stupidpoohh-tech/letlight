...
19. **끝 상태** — 하늘 / 지형 / 구름 / 짙게 젖은 땅 / 큰 나무
20. **백과사전** — 알아낸 것이 질문 · 세계 · 원리 세 축으로 쌓인다
...
아직 올라오지 않은 그림이 하나 있다. 빗물이 땅으로 들어가는 노드가 쓴다.

| 필요한 파일 | 쓰이는 순간 |
|---|---|
| `assets/soil-water.webp` | 지표 아래로 스며든 물 |

없어도 화면은 깨지지 않는다. 그 층만 조용히 빠진다
(`src/art/assets.js` 의 `WATER_LAYERS`).
같은 이유로 그 노드의 도감 도판(`plate: 'soil'`) 자리도 지금은 비어 있다.
