...
16. **세계의 변화 ⑤** — 물을 받아들인 땅의 색이 더 짙어진다.
    지표 아래의 물과 작은 물길은 그림이 올라오면 그 위에 얹힌다
...
아직 올라오지 않은 그림이 둘 있다. 빗물이 땅으로 들어가는 노드가 쓴다.

| 필요한 파일 | 쓰이는 순간 |
|---|---|
| `assets/soil-water.webp` | 지표 아래로 스며든 물 |
| `assets/surface-flow.webp` | 낮은 곳에 모인 작은 물길 |

둘 다 없어도 화면은 깨지지 않는다. 땅이 짙어지는 변화만 일어나고
그 층은 조용히 빠진다 (`src/art/assets.js` 의 `WATER_LAYERS`).
...
- **땅속의 물 그림** — `assets/soil-water.webp` · `assets/surface-flow.webp` 가
  올라오면 그대로 붙는다. 임시 그림은 만들지 않았다
