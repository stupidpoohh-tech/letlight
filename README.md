...
7. **지식** — 세계 위로 글 한 편이 올라온다.
   다 읽으면 `세계로 돌아가기`, 나오는 문제가 있으면 `한 번 더 생각해보기`
7-1. **나오는 문제** — 방금 읽은 것을 처음 보는 상황에 대 본다.
   맞혀야 `세계로 돌아가기` 가 생긴다. 그전에는 버튼 자체가 없다.
   아직 없는 노드도 있다. 없으면 7 에서 바로 세계로 간다
...
두 문제는 모양이 같고 화면도 같다. `exitQuiz` 를 써 넣으면 그 노드에만
한 걸음이 붙고, 없으면 글을 읽고 바로 세계로 돌아간다. UI 를 따로 만들 일은 없다.
