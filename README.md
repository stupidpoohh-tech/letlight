...
끄는 스위치는 도감 홈 맨 아래에 있다 (`boida.sound`).
다른 데를 보고 있으면 저절로 멈춘다.

**iOS 의 무음 스위치를 켜 두면 들리지 않는다.** 웹에서 우회할 방법이 없다.
...
**증발 질문은 아직 없다.** 고리 그림에는 자리를 남겨 두었고,
그 질문이 생기면 `requiredNodes` 에 id 한 줄만 더하면 된다.

잠열 질문(`waterLatentHeat`)도 원래는 그 증발 질문 다음에 와야 한다.
증발 질문이 없어 지금은 강 질문(`waterInfiltration`)이 바로 잇고 있다.
증발 노드가 생기면 `next` 한 줄만 그쪽으로 옮기면 된다.
